...
terraform {
  ...
  required_providers {
    ...
    cyberark_sia = {
      source  = "local/cyberark-sia" # Replace with registry path after publication
      version = "~> 1.0"
    }
  }
}
...
resource "cyberark_sia_database_workspace" "postgresql" {
...
resource "cyberark_sia_secret" "postgres_admin" {
  ...
  # depends_on = [cyberark_sia_database_workspace.postgresql]
}
...
output "sia_database_workspace_id" {
  ...
  value       = cyberark_sia_database_workspace.postgresql.id
}
...
output "integration_status" {
  ...
  value = {
    ...
    sia_workspace_name  = cyberark_sia_database_workspace.postgresql.name
    sia_secret_name     = cyberark_sia_secret.postgres_admin.name
    environment         = var.environment
