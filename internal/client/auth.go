...
func NewISPAuth(ctx context.Context, config *AuthConfig) (*ISPAuthContext, error) {
	...
	tflog.Debug(ctx, "Creating ISP authentication context", map[string]interface{}{
		"auth_method":      "IdentityServiceUser",
		"has_identity_url": config.IdentityURL != "",
	...
	tflog.Debug(ctx, "Authenticating with in-memory profile (force=true, bypass cache)")
	...
	if err != nil {
		tflog.Error(ctx, "ISP authentication failed", map[string]interface{}{
			"error": err.Error(),
		})
